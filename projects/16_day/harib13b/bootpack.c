...
void HariMain(void)
{
	...
	/*
	最开始的fifo32_init中指定任务的参数，我们用0代替了。因为我们在任务A中应用了休眠，也就需要使用让FIFO来唤醒的功能，不过在这个时间点上多任务的初始化 还没有完成，因此无法指定任务，只能先在这里用0代替，也就是禁用自动唤醒功能。	
	*/
	...
	*((int *) (task_b->tss.esp + 4)) = (int) sht_back;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			task_sleep(task_a); //MARK: 让任务休眠，让出cpu时钟周期资源。下一个我们关注的中断来了之后自然会被唤醒！当FIFO为空的时候，任务A将执行task_sleep来替代之前的HLT。
			io_sti();
		} else {
			i = fifo32_get(&fifo);
			io_sti();
