...
void HariMain(void)
{
	...
	// 段长限制为103字节，定义在gdt的3号和4号
	set_segmdesc(gdt + 3, 103, (int) &tss_a, AR_TSS32);
	set_segmdesc(gdt + 4, 103, (int) &tss_b, AR_TSS32);
	load_tr(3 * 8); // 给tr寄存器赋值，tr寄存器是用来存放tss的段号的,目前设置的是3号
	task_b_esp = memman_alloc_4k(memman, 64 * 1024) + 64 * 1024; // MARK: 这里为什么要加64*1024(计算出栈底的位置)
	...
	tss_b.edx = 0;
	tss_b.ebx = 0;
	tss_b.esp = task_b_esp; //MARK: 专门为任务B所定义的栈。有人可能会说，直接用任务A的栈不就好了吗?那可不行，如果真这么做的话，栈就会混成一团，程序也无法正常运行。
	tss_b.ebp = 0;
	tss_b.esi = 0;
	tss_b.edi = 0;
	tss_b.es = 1 * 8;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* �L�[�{�[�h�f�[�^ */
				...
				if (i < 0x54 + 256) {
					if (keytable[i - 256] != 0 && cursor_x < 144) { /* �ʏ핶�� */
						/* �ꕶ���\�����Ă���A�J�[�\����1�i�߂� */
						s[0] = keytable[i - 256];
						s[1] = 0;
				...
				taskswitch4();//MARK： 跳转到另一个段进行任务切换（按照TSS的描述）。 JMP 4*8:0
			} else if (i == 3) { /* 3�b�^�C�} */
