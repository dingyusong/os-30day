...
/// @brief MARK: 	将一个字符显示到屏幕上（需要有字体font数据）
/// @param vram 	显存映射首地址
/// @param xsize 	屏幕宽度
/// @param x 		描述一个字体需要的像素的宽度（多少个像素宽）
/// @param y 		描述一个字体需要的像素的宽度（多少个像素高）
/// @param c 		显示的颜色（就是屏幕上的那个像素的颜色）
/// @param font 	字体数据（绘制字体A所需要的数组的数据，数组的长度是像素高，每个数据的bit数是像素宽）
void putfont8(char *vram, int xsize, int x, int y, char c, char *font)
{
	int i;
	char *p, d /* data */;
