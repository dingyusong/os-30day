...
/// @brief 这个函数是按照CPU的规格要求，将段的信息归结成8个字节写入内存的。
/// 有以下三点 1：段的上限 2：段的基址 3：段的属性
/// @param sd 段描述符的地址
/// @param limit 段的上限，段的上限的一部分
/// @param base 段的基址，段的上限的一部分
/// @param ar  段的s属性，段的类型，DPL，P属性，段的上限的一部分
void set_segmdesc(struct SEGMENT_DESCRIPTOR *sd, unsigned int limit, int base, int ar)
{
	if (limit > 0xfffff) {
		ar |= 0x8000; /* G_bit = 1 */
